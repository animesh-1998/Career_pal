...
with open(r".\llm\prompts\job_hunt_prompt.txt", "r", encoding="utf-8") as f:
...
async def job_hunt_node(state: AgentState, tools: list) -> dict:

    # ── Guard: no tools available ────────────────────────────────
    if not tools:
        return {
            "messages": [AIMessage(content="LinkedIn tools are not available right now.")],
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        print(f"Job hunt ReAct iteration {iteration}")

        # Stuck detection — check across existing + new messages
        all_seen = existing_messages + new_messages
        if is_stuck(all_seen, tool_name="search_jobs", threshold=3):
            print("Job hunt node: stuck on search_jobs, aborting")
            stuck_msg = AIMessage(
                content="I tried searching for jobs multiple times but couldn't find "
                        "relevant results. Could you provide more details such as "
                        "location, experience level, or specific companies you're targeting?"
            )
            return {
        ...
        print(f"Job hunt ReAct: tool_calls={[c['name'] for c in (response.tool_calls or [])]}")

        # No tool calls → LLM is done reasoning
        if not response.tool_calls:
            print(f"Job hunt ReAct: completed in {iteration} iteration(s)")
            break
        ...
        for tool_call in response.tool_calls:
            ...
            print(f"Job hunt ReAct: calling '{tool_name}' args={tool_args}")
            ...
            if not tool:
                ...
                try:
                    ...
                    print(f"Job hunt ReAct: tool error — {e}")
            ...
            print(f"Job hunt ReAct: '{tool_name}' result preview: {str(result)[:200]}")
    ...
    if last_ai and "needs_clarification:" in last_ai.content.lower():
        ...
        print(f"Job hunt ReAct: runtime clarification needed — {question}")
        return {
