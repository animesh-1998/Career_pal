...
async def build_agent_graph():
    ...
    graph.add_node("tools", ToolNode(tools))          # ← ToolNode(tools) not just tools
    ...
    graph.add_conditional_edges(
        ...
        tools_condition,
        {
            "tools": "tools",                         # ← "tools" string not tools variable
            END: "chat_node"
        }
    )

    graph.add_edge("tools", "job_hunt_node")
